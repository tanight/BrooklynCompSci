...
export default {
  ...
  methods: {
    setTransform3D(el, degree, perspective, z) {
      ...
      z -= 5;
      el.style["-webkit-perspective"] = el.style["perspective"] = el.style[
        "-moz-perspective"
      ] =
        perspective + "px";
      el.style["-webkit-transform"] = el.style["transform"] =
        "rotateY(" + degree + "deg) translateZ(" + z + "px)";
    },
    displayIndex(imgSize, spacing, left, imgs, index, flat, width, titleBox) {
      var mLeft =
        (width - imgSize) * 0.5 - spacing * (index + 1) - imgSize * 0.5;
      ...
      for (var j = index + 1; j < imgs.length; ++j) {
        ...
        this.setTransform3D(
          imgs[j],
          flat ? 0 : (index - j) * 10 - 45,
          300,
          flat ? (index - j) * 10 : (index - j) * 30 - 20
        );
      }
    },
    ...
    initCoverFlow(c) {
      ...
      for (var j = 0; j < imgs.length; ++j) {
        ...
        imgs[j].style["transition"] =
          browserPrefix +
          "transform .4s ease, margin-left .4s ease, -webkit-filter .4s ease";
      ...
      placeholding.style.width =
        (width ? width * 2 : (imgSize + (imgs.length + 1) * spacing) * 2) +
        "px";
      ...
      if (shadow) {
        ...
        c.style["-webkit-perspective-origin"] = c.style[
          "perspective-origin"
        ] = c.style["-moz-perspective-origin"] = "50% 25%";
        for (var k = 0; k < imgs.length; ++k) {
          imgs[k].style.bottom = 20 + imgHeight + "px";
          imgs[k].style["-webkit-box-reflect"] =
            "below 0 -webkit-gradient(linear, 30% 20%, 30% 100%, from(transparent), color-stop(0.3, transparent), to(rgba(0, 0, 0, 0.8)))";
        }
      } else {
